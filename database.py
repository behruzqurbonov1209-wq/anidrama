...
class Database:
    ...
    def _init_db(self):
        with self._conn() as conn:
            conn.executescript("""
                ...
                -- Asosiy kontent jadvali (serial/anime/kino sarlavhasi)
    ...
    # ── CONTENT (serial/anime sarlavhasi) ──────────────────────────────

    def add_item(self, category: str, code: str, title: str, description: str = ""):
        """Faqat sarlavha qo'shadi — faylsiz"""
        with self._conn() as conn:
            conn.execute(
                """INSERT OR REPLACE INTO content (category, code, title, description)
                   VALUES (?, ?, ?, ?)""",
                (category, code.upper(), title, description)
            )
    ...
    def delete_item(self, category: str, code: str) -> bool:
        """Serialni va barcha qismlarini o'chiradi"""
        with self._conn() as conn:
            conn.execute(
                "DELETE FROM episodes WHERE category=? AND code=?",
                (category, code.upper())
            )
            cur = conn.execute(
                "DELETE FROM content WHERE category=? AND code=?",
                (category, code.upper())
            )
    ...
    def get_next_episode_num(self, category: str, code: str) -> int:
        """Keyingi qism raqamini avtomatik aniqlaydi"""
        with self._conn() as conn:
            row = conn.execute(
    ...
    def get_stats(self) -> dict:
        with self._conn() as conn:
            ...
            return {"users": users, "anime": anime, "drama": drama, "kino": kino, "episodes": ep_total}
